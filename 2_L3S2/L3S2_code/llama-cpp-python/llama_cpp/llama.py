...
class Llama:
    ...
    def __init__(
        ...
        *,
        ...
        # rope_scaling_type: Optional[int] = llama_cpp.LLAMA_ROPE_SCALING_TYPE_LINEAR,
    ...
    def _create_completion(
        ...
        if logprobs is not None:
            ...
            for idx, (token, token_str, logprobs_token) in enumerate(
                ...
                sorted_v, indices = torch.sort(logprobs_token_tmp,descending=True)
                sorted_logprobs = torch.cat((sorted_v.view(-1,1),indices.view(-1,1)),dim = 1)

                token_logprobs.append(logprobs_token[int(token)])
                top_logprob: Optional[Dict[str, float]] = {
                    # self.detokenize([i]).decode("utf-8", errors="ignore"): logprob
                    self.detokenize([int(i)]).decode("utf-8", errors="ignore"): logprob
                    for logprob, i in sorted_logprobs[:logprobs]
                }
                top_logprob.update({token_str: logprobs_token[int(token)]})
                top_logprobs.append(top_logprob)
