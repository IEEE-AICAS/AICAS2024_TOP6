...
GGML_CALL void ggml_rope_yarn_corr_dims(
    ...
    // dims[1] = MIN(n_dims - 1, end);
        // start and end correction dims
    float last_part = (2 * logf_fast(freq_base));
    // float last_part = (2 * logf(freq_base));
    float mid_part = 2 * (float)M_PI;

    float beta_fast11 = logf_fast(n_orig_ctx / (beta_fast * mid_part));
    float beta_slow11 = logf_fast(n_orig_ctx / (beta_slow * mid_part));
    // float beta_fast11 = logf(n_orig_ctx / (beta_fast * mid_part));
    // float beta_slow11 = logf(n_orig_ctx / (beta_slow * mid_part));
    
// #if defined(__ARM_FEATURE_SVE)
//     // single precision log
//     svbool_t pg = svptrue_b32();
//     svfloat32_t svx = svdup_n_f32(n_orig_ctx / (beta_fast * mid_part));
//     svfloat32_t svy = svdup_n_f32(n_orig_ctx / (beta_slow * mid_part));
//     svfloat32_t freq_base1 = svdup_n_f32(freq_base);

//     // _ZGVnN2v_logf
//     // _ZGVsMxv_logf
//     svfloat32_t beta_fast11_sve = _ZGVsMxv_logf(svx,pg);
//     svfloat32_t beta_slow11_sve = _ZGVsMxv_logf(svy,pg);
//     svfloat32_t freq_base2 = _ZGVsMxv_logf(freq_base1,pg);

//     // Use svextract_f32 to extract the result from the SVE vector
//     float beta_fast11 = svlastb_f32(pg,beta_fast11_sve);
//     float beta_slow11 = svlastb_f32(pg,beta_slow11_sve);
//     float freq_base2 = svlastb_f32(pg,freq_base2);
// #else
//     float beta_fast11 = logf(n_orig_ctx / (beta_fast * mid_part));
//     float beta_slow11 = logf(n_orig_ctx / (beta_slow * mid_part));
// #endif
    ...
    float start = floorf(n_dims * (beta_fast11) / last_part);
    float end = ceilf(n_dims * (beta_slow11) / last_part);
