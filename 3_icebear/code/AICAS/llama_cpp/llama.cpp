...
void llama_model_add_async_thread(const struct llama_model * model) {
    ggml_async_read_rearrange_weight((void *)model->ctxs[0]);
}

void llama_model_load_gemm_weight(const struct llama_model * model) {
    int continue_layer = 2;
    for (ggml_context *ctx : model->ctxs) {
        for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
             if ((cur->type == GGML_TYPE_Q4_0) && (cur->ne[1] % 4 == 0) && continue_layer-- <= 0) {
                rearrange_q4_0_weights_for_gemm(cur); // rearrange weights for GEMM MMLA kernels
                cur->weight_rearranged = true;
            } else if ((cur->type == GGML_TYPE_Q8_0) && (cur->ne[1] % 4 == 0) && continue_layer-- <= 0) {
                // printf("\"%s\", ", ggml_get_name(cur));
                // char file_name[128];
                // snprintf(file_name, 128, "re_weight/%s", ggml_get_name(cur));
                // FILE *fp = fopen(file_name, "wb");
                rearrange_q8_0_weights_for_gemm(cur); // rearrange weights for GEMM MMLA kernels
                cur->weight_rearranged = true;
                // fwrite(cur->rearranged_weight_gemm, ggml_nbytes(cur), 1, fp);
                // fclose(fp);
            }
        }
    }
...
void llama_model_free_gemm_weight(const struct llama_model * model) {
    for (ggml_context *ctx : model->ctxs) {
        for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
            if ((cur->type == GGML_TYPE_Q4_0 || cur->type == GGML_TYPE_Q8_0) && (cur->ne[1] % 4 == 0) && cur->weight_rearranged == true) {
                free(cur->rearranged_weight_gemm);
...
void llama_model_load_gemv_weight(const struct llama_model * model) {
    // FILE *fp = fopen("gemv_rearrange_wegiht.bin", "r");
    for (ggml_context *ctx : model->ctxs) {
        for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
            if ((cur->type == GGML_TYPE_Q4_0) && (cur->ne[1] % 4 == 0)) {
                rearrange_q4_0_weights_for_gemv(cur); 
                cur->weight_rearranged = true;
            } else if ((cur->type == GGML_TYPE_Q8_0) && (cur->ne[1] % 4 == 0)) {
                rearrange_q8_0_weights_for_gemv(cur); 
                cur->weight_rearranged = true;
            }
        }
    }
...
void llama_model_free_gemv_weight(const struct llama_model * model) {
    for (ggml_context *ctx : model->ctxs) {
        for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
            if ((cur->type == GGML_TYPE_Q4_0 || cur->type == GGML_TYPE_Q8_0) && (cur->ne[1] % 4 == 0)) {
                free(cur->rearranged_weight_gemv);
                cur->weight_rearranged = false;
            }
        }
    }
