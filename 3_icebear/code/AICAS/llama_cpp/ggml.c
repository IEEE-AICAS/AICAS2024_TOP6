...
pthread_mutex_t re_mtx = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t re_cond = PTHREAD_COND_INITIALIZER;
bool async_thread_init = false;
...
void rearrange_q4_0_weights_blocked8_neon(struct ggml_tensor * cur) {
    printf("rearrange_q4_0_weights_blocked8_neon ues %d\n", ggml_nbytes(cur) / 1024 / 1024);
...
void rearrange_q4_0_weights_blocked8_sve(struct ggml_tensor * cur) {
    ...
    printf("rearrange_q4_0_weights_blocked8_sve\n");
    if (svcntw() != 8) {
...
void rearrange_q4_0_weights_for_gemm(struct ggml_tensor * cur) {
    use_memory += ggml_nbytes(cur);
    printf("rearrange_q4_0_weights_for_gemm use %d M/n\n", use_memory / 1024 / 1024);
    int64_t nb = cur->ne[0] / QK4_0;
    uint32_t maby_use_memory = ggml_nbytes(cur);
    block_q4_0x4 * out_ptr_B = malloc(maby_use_memory);  // B_blocked->data;
...
void rearrange_q8_0_weights_blocked8_neon(struct ggml_tensor * cur) {
    printf("rearrange_q8_0_weights_for_gemv use %d M/n\n", use_memory / 1024 / 1024);
...
#if defined(__ARM_FEATURE_SVE)
    printf("rearrange_q8_0_weights_blocked8_sve\n");
    if (svcntw() != 8) {
        printf("ggml_gemv_q8_0_q8_0_blocked8_sve: SVE %d VL != 256 - aborting. Use Arm Neon GEMV kernels \n", svcntw());
        exit(1);
    }
...
void rearrange_q8_0_weights_for_gemm(struct ggml_tensor * cur) {
    use_memory += ggml_nbytes(cur);
    printf("rearrange_q8_0_weights_for_gemm use %d M/n\n", use_memory / 1024 / 1024);
...
UseGgmlGemm2:;
    ...
    if ((ggml_n_dims(src0) == 2) && (ne11 > 1)  && src0->weight_rearranged == true) {
        ...
        if (src0->type == GGML_TYPE_Q4_0) {
            ggml_gemm_q4_0_q8_0(ne00, ne11, ne01, ne11, (float *)((char *) dst->data), (const char *) src0->rearranged_weight_gemm, (const char *) wdata, ith, nth);
        } else if (src0->type == GGML_TYPE_Q8_0) {
            ggml_gemm_q8_0_q8_0(ne00, ne11, ne01, ne11, (float *)((char *) dst->data), (const char *) src0->rearranged_weight_gemm, (const char *) wdata, ith, nth);
        }
        // end = clock();
        // printf("use gemm tensor %s ne %d time %.2lfms\n", ggml_get_name(src0), ne11, ((double) (end - start)) / 1000);
    } 
    // else if ((ggml_n_dims(src0) == 2) && (ne11 == 1) && src0->weight_rearranged == true) {
    //     if (src0->type == GGML_TYPE_Q4_0) {
    ...
    // } 
        ...
        // end = clock();
        // printf("use origin tensor %s ne %d time %.2lfms\n", ggml_get_name(src0), ne11, ((double) (end - start)) / 1000);
    }
}
...
char *re_file[100] = 
{
"blk.0.attn_qkv.weight", "blk.0.attn_output.weight", "blk.0.ffn_gate.weight", "blk.0.ffn_up.weight", "blk.0.ffn_down.weight", "blk.1.attn_qkv.weight", "blk.1.attn_output.weight", "blk.1.ffn_gate.weight", "blk.1.ffn_up.weight", "blk.1.ffn_down.weight", "blk.2.attn_qkv.weight", "blk.2.attn_output.weight", "blk.2.ffn_gate.weight", "blk.2.ffn_up.weight", "blk.2.ffn_down.weight", "blk.3.attn_qkv.weight", "blk.3.attn_output.weight", "blk.3.ffn_gate.weight", "blk.3.ffn_up.weight", "blk.3.ffn_down.weight", "blk.4.attn_qkv.weight", "blk.4.attn_output.weight", "blk.4.ffn_gate.weight",
"blk.6.attn_output.weight", "blk.6.ffn_gate.weight", "blk.6.ffn_up.weight", "blk.6.ffn_down.weight", "blk.7.attn_qkv.weight", "blk.7.attn_output.weight", "blk.7.ffn_gate.weight", "blk.7.ffn_up.weight", "blk.7.ffn_down.weight", "blk.8.attn_qkv.weight", "blk.8.attn_output.weight", "blk.8.ffn_gate.weight", "blk.8.ffn_up.weight", "blk.8.ffn_down.weight", "blk.9.attn_qkv.weight", "blk.9.attn_output.weight", "blk.9.ffn_gate.weight", "blk.9.ffn_up.weight", "blk.9.ffn_down.weight", "blk.10.attn_qkv.weight", "blk.10.attn_output.weight", "blk.10.ffn_gate.weight", "blk.10.ffn_up.weight", "blk.10.ffn_down.weight", "blk.11.attn_qkv.weight", "blk.11.attn_output.weight", "blk.11.ffn_gate.weight", "blk.11.ffn_up.weight", "blk.11.ffn_down.weight", "blk.12.attn_qkv.weight", "blk.12.attn_output.weight", "blk.12.ffn_gate.weight", "blk.12.ffn_up.weight", "blk.12.ffn_down.weight", "blk.13.attn_qkv.weight", "blk.13.attn_output.weight", "blk.13.ffn_gate.weight", "blk.13.ffn_up.weight", "blk.13.ffn_down.weight", "blk.14.attn_qkv.weight", "blk.14.attn_output.weight", "blk.14.ffn_gate.weight", "blk.14.ffn_up.weight", "blk.14.ffn_down.weight", "blk.15.attn_qkv.weight", "blk.15.attn_output.weight", "blk.15.ffn_gate.weight", "blk.15.ffn_up.weight", "blk.15.ffn_down.weight", "blk.16.attn_qkv.weight", "blk.16.attn_output.weight", "blk.16.ffn_gate.weight", "blk.16.ffn_up.weight", "blk.16.ffn_down.weight", "blk.17.attn_qkv.weight", "blk.17.attn_output.weight", "blk.17.ffn_gate.weight", "blk.17.ffn_up.weight", "blk.17.ffn_down.weight", "blk.18.attn_qkv.weight", "blk.18.attn_output.weight", "blk.18.ffn_gate.weight", "blk.18.ffn_up.weight", "blk.18.ffn_down.weight", "blk.19.attn_qkv.weight", "blk.19.attn_output.weight", "blk.19.ffn_gate.weight", "blk.19.ffn_up.weight", "blk.19.ffn_down.weight", "output.weight"
};


void* thread_read_file(void* arg) {
    int fd, read_bytes;
    const struct ggml_context * ctx = arg;
    int read_file_index = 0;
    struct ggml_tensor *last = NULL;
    async_thread_init = true;
    while (true) {
        pthread_mutex_lock(&re_mtx);
        pthread_cond_wait(&re_cond, &re_mtx);
        read_file_index = read_file_index % 100;
        printf("ready load %s\n", re_file[read_file_index]);
        for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
            if (0 == strcmp(ggml_get_name(cur), re_file[read_file_index]) && cur->weight_rearranged == false) {
                char file_name[128];
                snprintf(file_name,128, "/mnt/tmpfs/re_weight/%s", re_file[read_file_index]);
                fd = open(file_name, O_RDONLY);
                int size = ggml_nbytes(cur);
                char * buffer = malloc(size);
                int bytesRead = read(fd, buffer, size);
                printf("load %s bytesRead %d M\n", re_file[read_file_index], bytesRead / 1024 / 1024);
                cur->rearranged_weight_gemm = buffer;
                cur->weight_rearranged = true;
                read_file_index++;
                break;
            }
        }
        pthread_mutex_unlock(&re_mtx);
    }
}

void load_re_weight() {
    pthread_cond_signal(&re_cond);
}

void ggml_async_read_rearrange_weight(void * ctx) {
    pthread_t thread_id;
    int rc = pthread_create(&thread_id, NULL, thread_read_file, ctx);
    if (rc != 0) {
