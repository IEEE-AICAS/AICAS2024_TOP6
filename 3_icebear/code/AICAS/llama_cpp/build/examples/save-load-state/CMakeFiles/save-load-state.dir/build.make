# CMAKE generated file: DO NOT EDIT!
# Generated by "Unix Makefiles" Generator, CMake Version 3.16

# Delete rule output on recipe failure.
.DELETE_ON_ERROR:


#=============================================================================
# Special targets provided by cmake.

# Disable implicit rules so canonical targets will work.
.SUFFIXES:


# Remove some rules from gmake that .SUFFIXES does not remove.
SUFFIXES =

.SUFFIXES: .hpux_make_needs_suffix_list


# Suppress display of executed commands.
$(VERBOSE).SILENT:


# A target that is always out of date.
cmake_force:

.PHONY : cmake_force

#=============================================================================
# Set environment variables for the build.

# The shell in which to execute make rules.
SHELL = /bin/sh

# The CMake executable.
CMAKE_COMMAND = /usr/bin/cmake

# The command to remove a file.
RM = /usr/bin/cmake -E remove -f

# Escaping for special characters.
EQUALS = =

# The top-level source directory on which CMake was run.
CMAKE_SOURCE_DIR = /root/AICAS/llama_cpp

# The top-level build directory on which CMake was run.
CMAKE_BINARY_DIR = /root/AICAS/llama_cpp/build

# Include any dependencies generated for this target.
include examples/save-load-state/CMakeFiles/save-load-state.dir/depend.make

# Include the progress variables for this target.
include examples/save-load-state/CMakeFiles/save-load-state.dir/progress.make

# Include the compile flags for this target's objects.
include examples/save-load-state/CMakeFiles/save-load-state.dir/flags.make

examples/save-load-state/CMakeFiles/save-load-state.dir/save-load-state.cpp.o: examples/save-load-state/CMakeFiles/save-load-state.dir/flags.make
examples/save-load-state/CMakeFiles/save-load-state.dir/save-load-state.cpp.o: ../examples/save-load-state/save-load-state.cpp
	@$(CMAKE_COMMAND) -E cmake_echo_color --switch=$(COLOR) --green --progress-dir=/root/AICAS/llama_cpp/build/CMakeFiles --progress-num=$(CMAKE_PROGRESS_1) "Building CXX object examples/save-load-state/CMakeFiles/save-load-state.dir/save-load-state.cpp.o"
	cd /root/AICAS/llama_cpp/build/examples/save-load-state && /usr/bin/c++  $(CXX_DEFINES) $(CXX_INCLUDES) $(CXX_FLAGS) -o CMakeFiles/save-load-state.dir/save-load-state.cpp.o -c /root/AICAS/llama_cpp/examples/save-load-state/save-load-state.cpp

examples/save-load-state/CMakeFiles/save-load-state.dir/save-load-state.cpp.i: cmake_force
	@$(CMAKE_COMMAND) -E cmake_echo_color --switch=$(COLOR) --green "Preprocessing CXX source to CMakeFiles/save-load-state.dir/save-load-state.cpp.i"
	cd /root/AICAS/llama_cpp/build/examples/save-load-state && /usr/bin/c++ $(CXX_DEFINES) $(CXX_INCLUDES) $(CXX_FLAGS) -E /root/AICAS/llama_cpp/examples/save-load-state/save-load-state.cpp > CMakeFiles/save-load-state.dir/save-load-state.cpp.i

examples/save-load-state/CMakeFiles/save-load-state.dir/save-load-state.cpp.s: cmake_force
	@$(CMAKE_COMMAND) -E cmake_echo_color --switch=$(COLOR) --green "Compiling CXX source to assembly CMakeFiles/save-load-state.dir/save-load-state.cpp.s"
	cd /root/AICAS/llama_cpp/build/examples/save-load-state && /usr/bin/c++ $(CXX_DEFINES) $(CXX_INCLUDES) $(CXX_FLAGS) -S /root/AICAS/llama_cpp/examples/save-load-state/save-load-state.cpp -o CMakeFiles/save-load-state.dir/save-load-state.cpp.s

# Object files for target save-load-state
save__load__state_OBJECTS = \
"CMakeFiles/save-load-state.dir/save-load-state.cpp.o"

# External object files for target save-load-state
save__load__state_EXTERNAL_OBJECTS =

bin/save-load-state: examples/save-load-state/CMakeFiles/save-load-state.dir/save-load-state.cpp.o
bin/save-load-state: examples/save-load-state/CMakeFiles/save-load-state.dir/build.make
bin/save-load-state: common/libcommon.a
bin/save-load-state: libllama.a
bin/save-load-state: examples/save-load-state/CMakeFiles/save-load-state.dir/link.txt
	@$(CMAKE_COMMAND) -E cmake_echo_color --switch=$(COLOR) --green --bold --progress-dir=/root/AICAS/llama_cpp/build/CMakeFiles --progress-num=$(CMAKE_PROGRESS_2) "Linking CXX executable ../../bin/save-load-state"
	cd /root/AICAS/llama_cpp/build/examples/save-load-state && $(CMAKE_COMMAND) -E cmake_link_script CMakeFiles/save-load-state.dir/link.txt --verbose=$(VERBOSE)

# Rule to build all files generated by this target.
examples/save-load-state/CMakeFiles/save-load-state.dir/build: bin/save-load-state

.PHONY : examples/save-load-state/CMakeFiles/save-load-state.dir/build

examples/save-load-state/CMakeFiles/save-load-state.dir/clean:
	cd /root/AICAS/llama_cpp/build/examples/save-load-state && $(CMAKE_COMMAND) -P CMakeFiles/save-load-state.dir/cmake_clean.cmake
.PHONY : examples/save-load-state/CMakeFiles/save-load-state.dir/clean

examples/save-load-state/CMakeFiles/save-load-state.dir/depend:
	cd /root/AICAS/llama_cpp/build && $(CMAKE_COMMAND) -E cmake_depends "Unix Makefiles" /root/AICAS/llama_cpp /root/AICAS/llama_cpp/examples/save-load-state /root/AICAS/llama_cpp/build /root/AICAS/llama_cpp/build/examples/save-load-state /root/AICAS/llama_cpp/build/examples/save-load-state/CMakeFiles/save-load-state.dir/DependInfo.cmake --color=$(COLOR)
.PHONY : examples/save-load-state/CMakeFiles/save-load-state.dir/depend

