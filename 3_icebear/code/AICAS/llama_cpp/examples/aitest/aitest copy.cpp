                ...
                while (true) {
                    ...
                    while (true) {
                        ...
                        if (!params.use_color) {
                            ...
                            LOG_TEE("%d %s", id, token_str.c_str());
                        }
                        ...
                        if (i_dft < (int) draft.size() && id == draft[i_dft]) {
                            ...
                            embd.push_back(id);
                            {
                                ...
                                const int64_t t_start_draft_us = ggml_time_us();
                                llama_ngram_cache_update(ngram_cache_context, LLAMA_NGRAM_MIN, LLAMA_NGRAM_MAX, embd, 1, false);
                            }
                    ...
                    if ((n_predict > 0 && n_predict >= max_new_token) || has_eos) {
                        break;
                    }
                    ...
                    GGML_ASSERT(draft[0] == embd.back());

                    llama_ngram_cache_draft(embd, draft, n_draft, LLAMA_NGRAM_MIN, LLAMA_NGRAM_MAX, ngram_cache_context, ngram_cache_dynamic, ngram_cache_static);
